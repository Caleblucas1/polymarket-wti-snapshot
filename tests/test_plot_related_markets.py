...
class RelatedMarketChartTests(unittest.TestCase):
    ...
    def test_write_related_market_chart_from_csvs(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            data_dir = Path(tmpdir)
            pairs_path = data_dir / "pairs.json"
            output = data_dir / "related.html"
            ...
            write_csv(
                data_dir / "left.csv",
                ["Deadline", "2026-07-17"],
                [{"Deadline": "July 31", "2026-07-17": "43"}],
            )
            write_csv(
                data_dir / "right.csv",
                ["Deadline", "2026-07-17"],
            ...
            with patch("plot_related_markets.load_registry", return_value=registry):
